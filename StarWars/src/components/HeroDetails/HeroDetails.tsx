...
} from "../../features/heroDetailsSlice/heroDetailsSlice";
import { fetchHeroById } from "../../features/allHeroesSlice/allHeroSlice";
...
const HeroDetails: React.FC = () => {
  ...
  const fetchHeroFilmDetails = async (hero: Hero | null): Promise<Film[]> => {
    ...
    const filmPromises = hero.films.map(
      (filmId: number, index: number) =>
        new Promise<Film>((resolve) => {
          setTimeout(async () => {
            const filmDetails = await dispatch(
   ...
   */
  const getCommonStarshipIds = (
    hero: Hero | null,
    filmDetails: Film[]
  ): number[] => {
    if (!hero || !hero.starships) return [];
    const starshipIdsFromFilms = new Set(
      filmDetails.flatMap((film) => film.starships)
    );
    const starshipIdsFromHero = new Set(hero.starships);
    return [...starshipIdsFromFilms].filter((id) =>
      starshipIdsFromHero.has(id)
    );
  };
   ...
   */
  const fetchHeroStarshipDetails = async (
    starshipIds: number[]
  ): Promise<Starship[]> => {
    const starshipPromises = starshipIds.map(
      (starshipId: number, index: number) =>
        new Promise<Starship>((resolve) => {
          setTimeout(async () => {
            const starshipDetails = await dispatch(
